...
class RobState(Node):
    ...
    def __init__(self):
        ...
        # Subscribe to the simulated state topic.
    ...
    def sync_state(self, msg: String):
        ...
        try:
            sim_state = RobotState(msg.data)
            
            if self.state != sim_state:
                self.get_logger().info(f'syncing state from simulation, now {sim_state.name}')
