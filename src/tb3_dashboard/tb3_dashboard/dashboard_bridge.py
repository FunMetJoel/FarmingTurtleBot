...
class DashboardBridgeNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('tank_capacity_liters', 5.0)
        ...
        self.tank_capacity = self.get_parameter('tank_capacity_liters').value
        ...
        self.pub_water_remaining = self.create_publisher(Float32, '/dashboard/water_remaining_pct', 10)
    ...
    def _publish_water_metrics(self):
        ...
        remaining_pct = max(0.0, (self.tank_capacity - water_dt) / self.tank_capacity * 100.0)
        self._pub_f32(self.pub_water_remaining, remaining_pct)

    def _publish_battery(self):
        elapsed_s = (self.get_clock().now().nanoseconds - self._start_ns) / 1e9
        battery = max(0.0, 100.0 - elapsed_s / 36.0)  # full drain in ~1h
        self._pub_f32(self.pub_battery, battery)
