...
# Bridges raw ROS topics (map, humidity, TF) into dashboard-friendly Float32 topics.
# Used when you want the dashboard metrics published on the bus without the HTTP server.
...
class DashboardBridgeNode(Node):
    def __init__(self):
        ...
        self.pub_water_saved_pct = self.create_publisher(
        ...
        self.create_subscription(
            OccupancyGrid, "/humidityMap", self._on_humidity_map, 10
        )
        ...
        self.create_subscription(
            BatteryState, "/battery_state", self._on_battery_state, sensor_qos
        )
    ...
    def _publish_battery(self):
        if self._battery_pct is not None:
            ...
            # 36s per percent: drains to 0 in around 1 hour of sim time
    ...
    def _pub_f32(self, publisher, value):
        # float32 is used everywhere in the dashboard to keep things simple
        msg = Float32()
