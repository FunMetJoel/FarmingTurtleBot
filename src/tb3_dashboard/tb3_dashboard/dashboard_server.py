...
class DashboardServerNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Float32, "/twin/limits/speed_scale", self._on_speed_scale, 10
        )
    ...
    def _on_nav_path(self, msg):
        poses = [
            {"pose": {"position": {"x": p.pose.position.x, "y": p.pose.position.y}}}
            for p in msg.poses
        ]
        with self._lock:
    ...
    def _on_alert(self, msg):
        with self._lock:
            ...
            if len(self._state["alerts"]) > 30:
                self._state["alerts"].pop(0)  # drop oldest when buffer full
    ...
    def _update_water_metrics(self):
        ...
        with self._lock:
            ...
            self._state["water_naive_liters"] = (
                total_cells * cell_area * self._water_rate
            )
...
def _grid_to_dict(msg):
    # converts OccupancyGrid to a plain dict that json.dumps can handle
    return {
        "info": {
